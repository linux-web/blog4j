...
public abstract class BlogHandler extends RequestHandler {
	...
	public void beforeRender(AbstractRender render) {
		// JSP/模板才把常用信息放入
		if (BeanUtils.isNot(render.getClass(), JSPRender.class, FreeMarkerRender.class))
			return;
		...
		// 所有页面
		context.getRoot().put("pages", postService.queryPublishPageByKeywords(null, new PageInfo(1, 10)));

		// 最新文章
		context.getRoot().put("latestPosts", postService.queryPublishLatestPost(10));
