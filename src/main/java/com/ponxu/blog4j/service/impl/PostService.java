...
public class PostService implements IPostService {
	...
	public int add(Post post, int[] tagIds) {
		...
		int id = DAO.execute(sql, post.getUrl(), post.getTitle(), post.getContent(), new Date(), post.getTop(),
				post.getStatus(), post.getType());
	...
	public int modify(Post post, int[] tagIds) {
		...
		int r = DAO.execute(sql, post.getUrl(), post.getTitle(), post.getContent(), post.getTop(), post.getStatus(),
	...
	public List<Post> queryPublishRelativePost(Post post, int count) {
		...
		condition += " and id in (select post_id from bj_post_tag where tag_id in" + tagids + " and post_id<>"
				+ post.getId() + ")";
		return query(condition, order, page, false);
	}
	 ...
	 * @return 已经设置好标签的文章列表
	 */
	private List<Post> query(String condition, String order, PageInfo page, boolean calcPage) {
		String sql = "select id,url,title,addtime,top,status,type,left(content," + Config.sublength
				+ ") as content from bj_post";
		...
		return populateTag(DAO.queryPage(POST_WRAPPER, sql, page.getCurrentIndex(), page.getPageSize()));
	}
	...
	private String keywordsCondition(String kw) {
		...
		for (String keywords : strs) {
			...
			condition.append("title like '%").append(keywords).append("%' or content like '%").append(keywords)
					.append("%'");
			i++;
		}
		condition.append(")");
