...
public class TagService implements ITagService {
	...
	public void removePostTag(int postid) {
		DAO.execute("update bj_tag set post_count=post_count-1" + " where post_count>0"
				+ " and exists (select tag_id from bj_post_tag" + " where tag_id=bj_tag.id and post_id=?)", postid);
